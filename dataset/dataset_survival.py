        ...
        if not self.use_h5:
            if self.data_dir:
                if self.mode == 'path':
                    ...
                    '''
                    wsi_path = find_file_by_slide_id(self.directory_path, case_id)
                    ...
                    #path_features = []
                    #directory_path = os.path.join(data_dir, 'pt_files1') #gbmlgg
                    #directory_path = os.path.join(data_dir, 'pt_files') #brca
                    #directory_path = data_dir #condensed brca feature
                    ...
                    #print(wsi_path)
                    path_features = torch.load(wsi_path, map_location=torch.device('cpu'))
                    #path_features = path_features.unsqueeze(0) #one more time for brca condensed feature
