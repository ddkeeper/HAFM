...
class TMILFusion_old(nn.Module):
    def __init__(self, dim=512):
...
class TMILFusion(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        h = torch.cat([h, x2_proj], dim=1)
        h = self.fusion_layer(h)
        h = h[:, :x1.shape[1], :]  # 保留x1对应长度的特征
        
        # 3. 位置信息编码
        H = h.shape[1]
        _H, _W = int(np.ceil(np.sqrt(H))), int(np.ceil(np.sqrt(H)))
        add_length = _H * _W - H
        h = torch.cat([h, h[:,:add_length,:]], dim=1)  # 补齐为方形
        
        # 4. 添加cls token并进行位置编码
        B = h.shape[0]
        cls_tokens = self.cls_token.expand(B, -1, -1).cuda()
        h = torch.cat((cls_tokens, h), dim=1)
        ...
        # 6. 规范化和提取最终特征
        h = self.norm(h)
        h = h[:, 1:x1.shape[1]+1, :]  # 移除cls token和补齐的tokens
