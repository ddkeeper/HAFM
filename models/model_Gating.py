...
#共享角色编码
class SharedModalRoleEncoding(nn.Module):
    def __init__(self, proj_dim=256, num_layers=4):
        super().__init__()
        # 所有层共享的基础角色编码
        #self.base_rel_pos_bias = nn.Parameter(torch.randn(2, proj_dim))
        #正交初始化
        self.rel_pos_bias = nn.Parameter(
            torch.nn.init.orthogonal_(torch.empty(2, proj_dim))
        )
    ...
    def forward(self, layer_idx=None):
        return self.base_rel_pos_bias * self.layer_scales[layer_idx].unsqueeze(-1)
...
class MM_CosineGate(nn.Module):
    ...
    layer_idx=0, role_encoding=None):
        super().__init__()
        self.bnum = branch_num
        ...
        # 根据参数选择角色编码方式
        self.role_encoding = IndependentModalRoleEncoding(proj_dim)
    ...
    def forward(self, x1, x2):
        ...
        # 计算特征平均值并添加模态角色编码
        rel_pos = self.role_encoding(self.layer_idx)
        x1_mean = x1_processed.mean(dim=1) + rel_pos[0]  # 添加待编码特征的位置编码
        x2_mean = x2_processed.mean(dim=1) + rel_pos[1]  # 添加参考特征的位置编码
        ...
        if self.training:
            ...
            #if (top_k > 1).any():
                #print(f"Found samples with multiple experts activated:")
                #print(f"top_k: {top_k}")
