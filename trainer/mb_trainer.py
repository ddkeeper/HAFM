...
def validate_survival_coattn_mb(cur, epoch, bs_micro, model, loader, n_classes, early_stopping=None, monitor_cindex=None, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., results_dir=None, args=None):
    ...
    if args.load_model and (args.model_type == "amfm" or args.model_type == "hmfm") and args.mome_gating_network == 'CosMLP':
        ...
        print(f"k_dist_G1: {k_dist_G1}")
        print(f"k_dist_P1: {k_dist_P1}")
        # 从各自的gating参数中获取expert_counts
        expert_dist_G1 = genom1_gp['expert_counts']
        expert_dist_P1 = patho1_gp['expert_counts']
        #print(f"expert_dist_G1: {expert_dist_G1}")
        #print(f"expert_dist_P1: {expert_dist_P1}")
        plot_expert_similarity(
            genom1_gp['sim_matrix'],
            patho1_gp['sim_matrix'],
            save_path=os.path.join(args.results_dir, f'expert_similarity_matrices.png')
        )
        plot_expert_activation_combined(
            genom1_gp['activation_gates'],
            patho1_gp['activation_gates'],
            save_path=os.path.join(args.results_dir, f'expert_activation_gates.png')
        )
...
def validate_grade_coattn_mb(cur, epoch, bs_micro, model, loader, n_classes, early_stopping=None, monitor_cindex=None, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., results_dir=None, args=None):
    ...
    #print(f"Average inference time per micro-batch: {avg_inference_time:.4f} seconds")
    CMoE_vis = None
    if args.load_model and args.model_type == "amfm" and args.mome_gating_network == 'CosMLP':
        CMoE_vis = model.get_gating_params()
        # CMoE_vis 包含了两个层的统计信息
        genom1_gp, patho1_gp = CMoE_vis
    ...
    return patient_results, acc, micro_auc, micro_ap, micro_f1, False, avg_inference_time, samples_expert_choices, CMoE_vis
