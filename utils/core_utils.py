        ...
        if args.task_type == 'survival':
            if args.mode == 'coattn':
                if args.model_type == 'mcat':
                    ...
                    #from trainer.mb_trainer_mixed import train_loop_survival_coattn_mb, validate_survival_coattn_mb
                    if not args.load_model: #ttrain mode or test mode
            ...
            if args.mode == 'coattn':
                if args.model_type == 'mcat':
                    ...
                    #mix precision training
                    #from trainer.mb_trainer_mixed import train_loop_grade_coattn_mb, validate_grade_coattn_mb
                    ...
                    for _ in range(times):
                        val_latest, acc, micro_auc, micro_ap, micro_f1, stop, inference_time, expert_choices, cmoe_vis = validate_grade_coattn_mb(cur, epoch, args.bs_micro, model, val_loader, args.n_classes, early_stopping, monitor_cindex, writer, loss_fn, reg_fn, args.lambda_reg, args.results_dir, args)
                        avg_inference_times.append(inference_time)
                        # 记录验证集性能
                        val_epochs.append(epoch)
                        val_accs.append(acc)
