...
def plot_expert_activation(activation_gates, layer_name, save_path):
    ...
    plt.close()
'''
def plot_expert_similarity(sim_matrix_G1, sim_matrix_P1, save_path=None):
    """
    在一张图中绘制两层的专家相似度矩阵
    Args:
    ...
    def compute_similarity(W):
        ...
        return torch.matmul(W_normalized.T, W_normalized).cpu().numpy()
    
    similarity_G1 = compute_similarity(sim_matrix_G1)
    similarity_P1 = compute_similarity(sim_matrix_P1)
    
    # 创建包含两个子图的图形
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
    
    # 绘制G1层相似度矩阵
    sns.heatmap(similarity_G1, 
                ax=ax1,
                annot=True,  
                fmt='.3f',   
                cmap='RdBu_r',
    ...
    ax1.set_title('Genom Layer')
    num_experts = similarity_G1.shape[0]
    expert_labels = [f'Expert {i+1}' for i in range(num_experts)]
    ax1.set_xticks(np.arange(num_experts) + 0.5)
    ax1.set_yticks(np.arange(num_experts) + 0.5)
    ax1.set_xticklabels(expert_labels, rotation=45)
    ax1.set_yticklabels(expert_labels, rotation=0)
    
    # 绘制P1层相似度矩阵
    sns.heatmap(similarity_P1, 
                ax=ax2,
                annot=True,  
                fmt='.3f',   
                cmap='RdBu_r',
    ...
    ax2.set_title('Patho Layer')
    ax2.set_xticks(np.arange(num_experts) + 0.5)
    ax2.set_yticks(np.arange(num_experts) + 0.5)
    ax2.set_xticklabels(expert_labels, rotation=45)
    ax2.set_yticklabels(expert_labels, rotation=0)
    
    plt.suptitle('Expert Similarity Matrices', fontsize=14, y=1.05)
    plt.tight_layout()
...
def plot_expert_activation_combined(activation_gates_G1, activation_gates_P1, save_path=None):
    """
    在一张图中绘制两层的专家激活阈值
    Args:
    ...
    """
    # 创建包含两个子图的图形
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(8, 6), gridspec_kw={'wspace': 0.3})
    
    # 将数据转换为numpy数组
    values_G1 = activation_gates_G1.detach().cpu().numpy().reshape(-1, 1)
    values_P1 = activation_gates_P1.detach().cpu().numpy().reshape(-1, 1)
    
    # 绘制G1层激活阈值
    sns.heatmap(values_G1,
                ax=ax1,
                cmap='RdBu_r',
                annot=True,
                fmt='.3f',
                cbar=False,
                center=0,  # 设置颜色分布的中心点
                xticklabels=['Genom Layer'],
                yticklabels=[f'Expert {i+1}' for i in range(len(activation_gates_G1))],
                square=True)
    ax1.set_title('Genom Layer')
    
    # 绘制P1层激活阈值
    sns.heatmap(values_P1,
                ax=ax2,
                cmap='RdBu_r',
                annot=True,
                fmt='.3f',
                cbar=False,
                center=0,  # 设置颜色分布的中心点
                xticklabels=['Patho Layer'],
                yticklabels=[f'Expert {i+1}' for i in range(len(activation_gates_P1))],
                square=True)
    ax2.set_title('Patho Layer')
    
    plt.suptitle('Expert Activation Gates', fontsize=14, y=1.02)
    plt.tight_layout()
...
def plot_expert_k_distribution(k_dist_G1, k_dist_P1, save_path=None):
    ...
    print("\n专家激活数量分布统计:")
    print("\nG1层激活专家数量分布:")
    for i, count in enumerate(k_dist_G1):
        if count > 0:  # 只打印有激活的数量
            print(f"激活{i}个专家的样本数: {count.item() if torch.is_tensor(count) else count}")
    
    print("\nP1层激活专家数量分布:")
    for i, count in enumerate(k_dist_P1):
        if count > 0:  # 只打印有激活的数量
            print(f"激活{i}个专家的样本数: {count.item() if torch.is_tensor(count) else count}")
